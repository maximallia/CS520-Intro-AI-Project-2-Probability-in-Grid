 ...
 * Represents the maze used by Agent 9 
...
public class Maze {
    ...
    /**
     * Method to move the target in the maze randomly in one of the four cardinal directions.
     * The direction is randomly chosen, and we check to make sure that the proposed cell isn't
    ...
    public void moveTarget() {
    	...
    	while (!targetMoved) {
    		...
    		if (rand == 0) {
    			directionOfTravel = new Point((int) target.getX(), (int) target.getY() - 1);
    		} else if (rand == 1) {
    			directionOfTravel = new Point((int) target.getX() - 1, (int) target.getY());
    		} else if (rand == 2) {
    			directionOfTravel = new Point ((int) target.getX() + 1, (int) target.getY());
    		} else {
    			directionOfTravel = new Point((int) target.getX(), (int) target.getY() + 1);
    		...
    		if (inBounds(directionOfTravel)) {
    			if (getCell((int) directionOfTravel.getX(), (int) directionOfTravel.getY()).getTerrain().value != 3) {
    				this.target = directionOfTravel;
    				targetMoved = true;
    			}
    		}
     ...
     * @see					Agent9#insertCell(CellInfo, ArrayList)
     */
